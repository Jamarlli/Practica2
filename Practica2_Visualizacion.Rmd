...
#author: "Javier Martinez Llinares"
#date: "`r Sys.Date()`"
output:
  html_document:
    echo: yes
...
load("data/weatherAUS_distributions.RData") # Cargamos el fichero ``weatherAUS.RData``
library("ggplot2")

# Creamos el grafico con x = MinTemp e y = MaxTemp
grafico1 <- ggplot(data = ds, aes(x = MinTemp, y = MaxTemp)) 
grafico1

# Añadimos una capa para contruir un grafico de dispersion de x e y
grafico1 <- grafico1 + geom_point()
grafico1

# Generamos el mismo grafico cambiando aes para que el color dependa de la 
# variable "RainTomorrow"
grafico2 <- ggplot(data = ds, aes(x = MinTemp, y = MaxTemp, color = RainTomorrow))
grafico2

# A partir de grafico2 añadimos una capa para generar el grafico de dispersion
grafico2 <- grafico2 + geom_point()
grafico2

# Añadimos un titulo al grafico2 guardandolo en grafico3
grafico3 <- grafico2 + ggtitle("Prevision de lluvia con máximas 
y minimas temperaturas")
grafico3

#Almacenamos el grafico resultante en un pdf
ggsave("./figure/Ejercicio1.pdf", width = 5, height = 5)
```
...
#Creamos un grafico de dispersion con x = Temp9am e y = Pressure9am
grafico2_1 <- ggplot(data = ds, aes(x = Temp9am, y = Pressure9am)) + 
  geom_point()
grafico2_1

# Modificamos el grafico anterior para que la forma del punto dependa de la 
# variable RainToday
grafico2_2 <- ggplot(data = ds, aes(x = Temp9am, y = Pressure9am)) + 
  geom_point(aes(shape = RainToday))
grafico2_2

# Modificamos el grafico anterior para que la forma del punto dependa de la 
# variable RainToday

grafico2_3 <- ggplot(data = ds, aes(x = Temp9am, y = Pressure9am)) + 
  geom_point(aes(color = RainToday)) 
# Añadimos color al grafico2_1
grafico2_3

# Se observa mejor la informacion de RainToday con el color, ya que con la forma
# no se distinguen bien unos puntos de otros debiado a que estan muy amontonados

#Añadimos un titulo al grafico
grafico2_4 <- grafico2_3 + ggtitle("Prevision de lluvia sabiendo la presion 
y la temperatura a las 9:00")
grafico2_4

#Almacenamos el grafico resultante en un pdf
ggsave("./figure/Ejercicio2.pdf", width = 5, height = 5)
```
...
load("data/iris_tidy.RData") # Cargamos el fichero ``iris_tidy.RData``

# Con la funcion str() comprobamos la estructura de iris.tidy
str(iris.tidy) # Iris.tidy contiene 4 variables (Species, Part, Measure, Value)

# Creamos un grafico ggplot con x = Species e y = Value
iris.grafico1 <- ggplot(data = iris.tidy, aes(x = Species, y = Value))
iris.grafico1

# Añadimos una capa para crear un grafico de dispersion de Specie y Value
iris.grafico1 <- iris.grafico1 + geom_point()
iris.grafico1

# Modificamos grafico 1 para que de informacion acerca de la variable Part
iris.grafico2 <- ggplot(data = iris.tidy, aes(x = Species, y = Value, 
                                              color = Part)) + geom_point()
iris.grafico2

# Introducimos la informacion de la variable Measure
iris.grafico2_cols <- iris.grafico2 + facet_grid(.~Measure )
iris.grafico2_cols
iris.grafico2_filas <- iris.grafico2 + facet_grid(Measure~. )
iris.grafico2_filas
# Es mucho mas facil comparar las dos variables con la disposicion de columnas,
# ya que se diferencian mas facilmente los puntos

# Cambiamos la capa geom_point() por la geom_jitter()

iris.grafico3 <- ggplot(data = iris.tidy, aes(x = Species, y = Value, color = Part)) + 
  geom_jitter() + facet_grid(.~Measure )
iris.grafico3
# La diferencia es que con geom_jitter los puntos no estan sobre un eje sino que
# se reparten en un espacio asignado a su especie, lo que facilita la compresion

#Almacenamos el grafico resultante en un pdf
ggsave("./figure/Ejercicio3.pdf", width = 5, height = 5)
```
...
load("data/Ejercicio4_Data.Rdata") # Cargamos el fichero ``Ejercicio4_Data.Rdata``

# Representamos un grafico de puntos 
grafico_puntos <- ggplot(data = DatosEjer4, 
                  aes(x = Fecha, y = Ventas, color = Fabricante)) +geom_point()
grafico_puntos

# Modificamos el grafico anterior para unir los puntos con lineas
grafico_conlineas <- grafico_puntos + geom_line()
grafico_conlineas

# Modificamos el grafico anterior para cambiar el tipo de linea segun Fabricante
grafico_conlineas2 <- grafico_puntos + geom_line(aes(linetype = Fabricante))
grafico_conlineas2

# Añadimos el título “Ventas mensuales de diferentes compañías tecnológicas”.
grafico_conlineas3 <- grafico_conlineas2 + ggtitle("Ventas mensuales de diferentes 
compañías tecnológicas")
grafico_conlineas3

#Almacenamos el grafico resultante en un pdf
ggsave("./figure/Ejercicio4.pdf", width = 5, height = 5)
```
...
load("data/Experimento.Rdata") # Cargamos el fichero ``Experimento.Rdata``

# Representamos un grafico de puntos 
grafico5_1 <- ggplot(data = Experimento, aes(x = Dosis, y = Altura, 
linetype = Complemento, shape = Complemento)) + geom_point() + geom_line()

grafico5_1

#Modificamos el grafico anterior añadiendo barras de error
grafico5_2 <- grafico5_1 + geom_errorbar(aes(ymin=Altura-Error_Altura, 
                                        ymax=Altura+Error_Altura), width=.1,)

grafico5_2

# Modificamos el tipo de linea y el color del grafico anterior
grafico5_3 <- ggplot(data = Experimento, aes(x = Dosis, y = Altura, col = Complemento)) + 
geom_point(aes(shape = Complemento)) + geom_line(aes(linetype = Complemento)) + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1,) + scale_color_brewer(palette="Reds")

grafico5_3

#Almacenamos el grafico resultante en un pdf
ggsave("./figure/Ejercicio5.pdf", width = 5, height = 5)
```
...
# Cargamos el fichero ``weatherAUS_distributions.RData``
load("data/weatherAUS_distributions.RData") 

# Creamos un grafico con los boxplots de MinTemp para cada valor de Year
g6.1 <- ggplot(data = ds, aes(x = Year, y = MinTemp)) + geom_boxplot()
g6.1

# Creamos un grafico con los violines de MaxTemp para cada valor de Year
g6.2 <- ggplot(data = ds, aes(x = Year, y = MaxTemp)) + geom_violin()
g6.2

# Añadimos a g6.2 una capa de etiquetas de título
g6.3 <- g6.2 + labs(title = "Distribución de temperaturas máximas por año", x = "Año", y = "Temperatura (ºC)")
g6.3


# Eliminamos la leyenda del grafico anterior y añadimos los años
# Dado que no hay leyenda lo coloreamos para forzar que haya
g6.4 <- ggplot(data = ds, aes(x = Year, y = MaxTemp)) + geom_violin(aes(color = Year)) + 
theme(axis.text.x = element_text(angle = 20, size = 6,hjust = 1, vjust = 1), legend.position='none')
g6.4

#Almacenamos el grafico resultante en un pdf
ggsave("./figure/Ejercicio6.pdf", width = 5, height = 5)
```
...
# Cargamos el fichero
fich <- read.table("http://assets.datacamp.com/blog_assets/chol.txt", header = TRUE)

# Generamos el histograma de height
g7.1 <- ggplot(data = fich, aes(x = HEIGHT)) + geom_histogram()
g7.1

# Añadimos titulo
g7.2 <- g7.1 + labs(title = "Histograma de la variable Altura", 
                    x = "Altura (cm)", y = "Frecuencia")
g7.2

# Ajustamos el rango y los bins
g7.3 <- ggplot(data = fich, aes(x = HEIGHT)) + geom_histogram(bins = 20, breaks = seq(155, 195)) + 
labs(title = "Histograma de la variable Altura", x = "Altura (cm)", y = "Frecuencia")
g7.3

# Cambiamos el color del histograma 
g7.4 <- ggplot(data = fich, aes(x = HEIGHT)) + geom_histogram(bins = 20, breaks = seq(155, 195),col="red",fill="blue",alpha = .5) + 
labs(title = "Histograma de la variable Altura", x = "Altura (cm)", y = "Frecuencia")
g7.4  

# Añadimos la curva de distribucion
g7.5 <- ggplot(data = fich, aes(x = HEIGHT)) + geom_histogram(aes(y=stat(density)),bins = 20, breaks = seq(155, 195),col="red",fill="blue",alpha = .5) + 
labs(title = "Histograma de la variable Altura", x = "Altura (cm)", y = "Frecuencia")+ geom_density(col="green")
g7.5

# Representamos g7.5 para cada valor de Smoke
g7.6 <- g7.5 + facet_grid(SMOKE~.)
g7.6

#Almacenamos el grafico resultante en un pdf
ggsave("./figure/Ejercicio7.pdf", width = 5, height = 5)
```
